...
def create_empty_dataframe() -> pd.DataFrame:
    ...
    return pd.DataFrame({
        ...
        'e3fp_fp': pd.Series(dtype='object'),
        'ecfp_fp': pd.Series(dtype='object'),
        'electroshape_fp': pd.Series(dtype='object'),
        'functional_groups_fp': pd.Series(dtype='object'),
        'maccs_fp': pd.Series(dtype='object'),
        'pattern_fp': pd.Series(dtype='object'),
        'pharmacophore_fp': pd.Series(dtype='object'),
...
def add_grade_columns(df: pd.DataFrame, extended: bool = False) -> pd.DataFrame:
    """
    Add GRADE descriptor columns to DataFrame.
    ...
    try:
        from features.grade_descriptors.grade_descriptors import get_grade_descriptor_names
        descriptor_names = get_grade_descriptor_names(extended)
        ...
        logger.warning("Could not import GRADE descriptors module")
...
def load_sdf(filepath) -> pd.DataFrame:
    ...
    try:
        ...
        for col in ['mapchiral_fp', 'e3fp_fp', 'ecfp_fp', 
                    'electroshape_fp', 'functional_groups_fp', 'maccs_fp', 'pattern_fp', 
                    'pharmacophore_fp', 'interaction_fp', 'interactions', 'grade', 'prediction']:
            df[col] = None

        for col in ['num_interactions', 'clashes']:
            df[col] = 0

        for col in ['score', 'strain_energy']:
            df[col] = 0.0
...
def process_score_column(
    ...
    logger.info(f"Processed scores from '{score_label}' column (direction: {score_direction})")
    return df
