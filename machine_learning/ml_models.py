...
def get_available_fingerprints(df: pd.DataFrame, fingerprint_config: Optional[Dict[str, Any]] = None) -> Dict[str, bool]:
	"""
	Determine which fingerprints are both enabled in config and actually available in the data.
	
	Args:
		df: Molecules DataFrame
		fingerprint_config: Fingerprint configuration dict (from session config)
		
	Returns:
		Dict mapping fingerprint names to boolean availability
	"""
	from analysis.similarity import get_enabled_fingerprint_columns
	import streamlit as st
	
	# Get fingerprint config - fallback to session state if not provided
	if fingerprint_config is None:
		if hasattr(st, 'session_state') and hasattr(st.session_state, 'config'):
			fingerprint_config = st.session_state.config.get('fingerprint_config', {})
		else:
			# Default fallback - enable common fingerprint types
			fingerprint_config = {
				'compute_ecfp': True,
				'compute_functional_groups': True,
				'compute_maccs': True
			}
	
	# Get enabled fingerprint columns from config
	enabled_columns = get_enabled_fingerprint_columns({'fingerprint_config': fingerprint_config})
	
	# Check data availability for each fingerprint type
	available_fingerprints = {}
	
	# Interaction fingerprints (always include if data exists)
	if 'interaction_fp' in df.columns:
		available_fingerprints['interaction_fp'] = df['interaction_fp'].notna().any()
	else:
		available_fingerprints['interaction_fp'] = False
	
	# Standard fingerprints - only include if enabled in config AND available in data
	for column in enabled_columns:
		if column in df.columns:
			available_fingerprints[column] = df[column].notna().any()
		else:
			available_fingerprints[column] = False
	
	# Legacy fingerprint support - detect and use legacy fingerprints if new ones are not available
	legacy_fingerprint_mapping = {
		'morgan_fp': 'ecfp_fp',  # Morgan is similar to ECFP
		'rdkit_fp': 'maccs_fp'   # Use RDKit as alternative to MACCS
	}
	
	# If no modern fingerprints are available, check for legacy ones
	modern_fps_available = any(available_fingerprints.values())
	if not modern_fps_available:
		for legacy_col, modern_equivalent in legacy_fingerprint_mapping.items():
			if legacy_col in df.columns and df[legacy_col].notna().any():
				available_fingerprints[legacy_col] = True
				logger.info(f"Using legacy fingerprint {legacy_col} as fallback")
	
	logger.info(f"Available fingerprints: {[k for k, v in available_fingerprints.items() if v]}")
	return available_fingerprints


def prepare_features_from_dataframe(df: pd.DataFrame, 
									fingerprint_selection: Dict[str, bool]) -> Tuple[np.ndarray, List[int]]:
	"""
	Extract feature matrix from molecules DataFrame for ML training/prediction.
	
	Args:
		df: Molecules DataFrame
		fingerprint_selection: Dict mapping fingerprint column names to boolean selection
	...
	if not any(fingerprint_selection.values()):
	...
	# Apply mask for each selected fingerprint type
	for fp_column, is_selected in fingerprint_selection.items():
		if is_selected and fp_column in df.columns:
			valid_mask = valid_mask & df[fp_column].notna()
	...
	for _, row in valid_df.iterrows():
		...
		# Process each selected fingerprint type
		for fp_column, is_selected in fingerprint_selection.items():
			if is_selected and fp_column in valid_df.columns and row[fp_column] is not None:
				if fp_column == 'interaction_fp':
					# Handle interaction fingerprints (JSON format)
					try:
						ifp_data = json.loads(row[fp_column])
					...
					# Handle other fingerprint types (array format)
					feature_vector.extend([float(x) for x in row[fp_column]])
	...
	# Create display names for selected fingerprints
	fp_display_names = {
		'interaction_fp': 'Interaction',
		'e3fp_fp': 'E3FP',
		'ecfp_fp': 'ECFP',
		'electroshape_fp': 'ElectroShape',
		'functional_groups_fp': 'FunctionalGroups',
		'maccs_fp': 'MACCS',
		'pattern_fp': 'Pattern',
		'pharmacophore_fp': 'Pharmacophore'
	}
	
	selected_fps = [fp_display_names.get(fp_col, fp_col) for fp_col, is_selected in fingerprint_selection.items() if is_selected]
...
def train_model(df: pd.DataFrame, 
	...
	Args:
		...
		fingerprint_config: Configuration dict for fingerprint selection (uses session config if None)
	...
	# Get available fingerprints based on config and data availability
	available_fingerprints = get_available_fingerprints(graded_df, fingerprint_config)
	
	# Filter to only include available fingerprints
	fingerprint_selection = {k: v for k, v in available_fingerprints.items() if v}
	
	if not fingerprint_selection:
		raise ValueError("No valid fingerprints available for training. Check your data and fingerprint configuration.")
	...
	if feature_engineering_config is not None:
		...
		X, feature_metadata = engineer_features_for_training(
			...
			config=fingerprint_selection
		)
	else:
		# Use standard feature preparation
		X, _ = prepare_features_from_dataframe(graded_df, fingerprint_selection)
