...
def create_hybrid_fingerprint_features(df: pd.DataFrame,
    ...
    Args:
        ...
        use_ecfp: Include ECFP fingerprints (replaces deprecated morgan)
        use_maccs: Include MACCS fingerprints (replaces deprecated rdkit)
    ...
    for idx, row in df.iterrows():
        ...
        if use_ecfp and row['ecfp_fp'] is not None:
            ecfp_fp = [float(x) for x in row['ecfp_fp']]
            mol_features.extend(ecfp_fp)
            if idx == 0:  # Only add names once
                feature_names.extend([f'ecfp_{i}' for i in range(len(ecfp_fp))])
        
        if use_maccs and row['maccs_fp'] is not None:
            maccs_fp = [float(x) for x in row['maccs_fp']]
            mol_features.extend(maccs_fp)
            if idx == 0:
                feature_names.extend([f'maccs_{i}' for i in range(len(maccs_fp))])
...
def engineer_features_for_training(df: pd.DataFrame,
    ...
    use_ecfp = config.get('use_ecfp_fp', True)
    use_maccs = config.get('use_maccs_fp', True)
    ...
    X, feature_names = create_hybrid_fingerprint_features(
        df, use_ecfp, use_maccs, use_interaction, use_mapchiral
    )
    ...
    if use_importance_selection and 'grade' in df.columns:
        ...
        from machine_learning.encoding.encodings import encode_sequential
